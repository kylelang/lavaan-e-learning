...
data as *holzinger_swineford.rds*. 

These data contain a number of educational test scores from 7th and 8th grade 
students in two schools. These data contain `r nrow(hs)` observations of the 
...
The first model we will consider is a simple mediation model wherein the effect 
of *student age* on *spatial reasoning ability* is mediated by *mathematical 
ability*.
...
$\hat{ab} = `r round(out$est, 3)`$, 
$SE_{ab} = `r round(out$se, 3)`$, 
$Z = `r round(z, 3)`$, 
$p = `r round(p, 3)`$
). 
a. There is a significant, negative effect of age on spatial reasoning ability 
transmitted through mathematical ability.
a. The normality assumption underlying Sobel test may not be appropriate. We 
...
resamples.
- Test the significance of the indirect effect via the 95% BCa CI.
...
a. Is the indirect effect significant according to the 95% *BCa CI*?
...
percentile CI (
$\hat{ab} = `r round(out1$est, 3)`$, 
$95\%~CI_{perc} = [`r round(out1$ci.lower, 3)`; `r round(out1$ci.upper, 3)`]$
). 
a. Yes, the indirect effect is statistically significant according to the 
BCa CI (
$\hat{ab} = `r round(out2$est, 3)`$, 
$95\%~CI_{BCa} = [`r round(out2$ci.lower, 3)`; `r round(out2$ci.upper, 3)`]$
). 
a. The 95% BCa CI for the indirect effect does not contain zero, so we can infer 
...
according to the 95% BCa CI?
a. Is the specific indirect effect through *verbal ability* significant 
according to the 95% BCa CI?
a. Is the total indirect effect significant according to the 95% BCa CI?
...
$\hat{ab} = `r round(out1$est, 3)`$, 
$95\%~CI_{BCa} = [`r round(out1$ci.lower, 3)`; `r round(out1$ci.upper, 3)`]$
). 
...
$\hat{IE} = `r round(out3$est, 3)`$, 
$95\%~CI_{BCa} = [`r round(out3$ci.lower, 3)`; `r round(out3$ci.upper, 3)`]$
). 
...
data are available as *eating_attitudes_completed.rds*.
...
You can download the original data [here][ea_data], and you can access the code 
...
significant according to the 95% BCa CI?
...
and *Anxiety* significant according to the 95% BCa CI?
a. Is the total indirect effect significant according to the 95% BCa CI?
a. Briefly interpret the indirect effects.
...
significant (
$\hat{adb} = `r round(out3$est, 3)`$, 
$95\%~CI_{BCa} = [`r round(out3$ci.lower, 3)`; `r round(out3$ci.upper, 3)`]$
). 
a. Yes, the total indirect effect is statistically significant (
$\hat{IE} = `r round(out4$est, 3)`$, 
$95\%~CI_{BCa} = [`r round(out4$ci.lower, 3)`; `r round(out4$ci.upper, 3)`]$
). 
...
food.
   ...
   nonsignificant indirect effect and, hence, not supported by these results.
```
...
[hs_code]: https://github.com/kylelang/lavaan-e-learning/tree/main/code
[amda]: https://www.cms.guilford.com/books/Applied-Missing-Data-Analysis/Craig-Enders/9781606236390
[ea_data]: https://www.appliedmissingdata.com/analyses
[ea_code]: https://github.com/kylelang/lavaan-e-learning/tree/main/code