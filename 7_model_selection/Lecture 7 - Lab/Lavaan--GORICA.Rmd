---
title: "Lab: Model Selection in SEM using the GORICA"
...
#library(gorica)
#
# Install and load the bain library because it contains the data 'sesamesim'
...
You can compare your path diagram to Figure 1 in the article: https://doi.org/10.1080/10705511.2020.1836967; or compare it to one created based on the fitted object in Exercise 1d.
...
*Make sure* to install and load all the packages needed (i.e., lavaan, restriktor, and gorica/bain).
...
#library(gorica)
#
# Also install and load the bain library because it contains the data 'sesamesim'
if (!require("bain")) install.packages("bain") # install this package first (once)
library(bain)
```
...
* In case there is one hypothesis of interest, then your failsafe or competing hypothesis is preferably its complement. Then, by default: `comparison = "complement"`
...
results1 <- goric(fit1, hypotheses = list(H1.1 = H1.1), 
                  #comparison = "complement", # default if one hypothesis
                  #type = "gorica", # default if lavaan object
                  standardized = TRUE) 
```
...
The order-restricted hypothesis H1.1 has 
`r round(results1$ratio.gw[1,2], 2)`
...
* The GORICA weight for H1 (against it complement) is 
`r round(results1$result[1,7], 3)`.
* Hence, the support in the data in favor of H1.1 is overwhelming: H1.1 is 
`r round(results1$result[1,7], 3)`
$/$
`r round(results1$result[2,7], 3)`
$\approx$ 
`r round(results1$ratio.gw[1,2], 0)` 
times more supported than its complement.
...
You can compare your path diagram to 
<!-- Figure 2 in the article: https://doi.org/10.1080/10705511.2020.1836967; or compare it to  -->
the one created based on the fitted object in Exercise 2d.
...
Notably, only in case all these hypotheses are of interest, these should all be included in the set; especially if there is overlap like here (see the article: https://doi.org/10.1080/10705511.2020.1836967 and/or 'Guidelines_output_GORIC.html' on https://github.com/rebeccakuiper/Tutorials for more information). 
...
Note: In case of equal log-likelihood (loglik) values (i.e., ratio of loglik weights of 1), the ratio of GORICA weights are solely based on the difference in penalty values. See the article (https://doi.org/10.1080/10705511.2020.1836967) or the guidelines ('Guidelines_output_GORIC.html' on https://github.com/rebeccakuiper/Tutorials) for more information.

&nbsp;

**Possible conclusions**

* Since (at least one of) the theory-based/informative hypotheses obtains more support than the unconstrained, we can meaningfully compare the theory-based/informative hypotheses. That is, we do not select the best from a set of weak hypotheses.  
...
* Thus, there is support for the hypothesis that a larger score on B corresponds to a larger score on A (i.e., a positive effect) and that age and peabody do not predict A. Based on the evaluation below, this hypothesis is approximately 7 
<!-- nb object results2_c$ratio.gw[1,2] nog niet bekend :-) -->
times more likely than its complement containing all competing hypotheses.

&nbsp;

**Extra: H1.2 vs its complement**

Since the hypotheses are nested, the hypotheses share support. Therefore, we also examine the best of these (i.e., H1.2) against its complement:
...
The order-restricted hypothesis ‘H1.2’ has  
`r round(results2_c$ratio.gw[1,2], 2)`
times more support than its complement. 
<!-- This is convincing support for H1.2. -->

Note: The log-likelihood (loglik) weights seem to be quite close. This could then indicate that one or more of the inequality constraints can be replaced by (about-)equality constraints (leading to one or more boundary hypotheses). One could investigate with the benchmarks function, using 'output_type = "rlw"', whether the loglik weights indeed are close.  
*However*, in this case, there is already at least one equality (namely 2: "AB > APeabody = AAge = 0"). Thus, this hypothesis is a possible boundary hypothesis. Therefore, the (seemingly) close loglik weights can be due to that (so, there does not need to be another equality). You could check (using 'output_type = "glw"', which is the default) for multiple boundary hypotheses whether indeed the obtained ratio of GORICA weights fits the distribution based on this hypothesis better than the distributions of the other boundary hypotheses (here, "AB = APeabody = AAge = 0").  
Notably, replacing the equality by an about-equality may solve the problem. Hence, whenever you are interested in an equality and you can specify a range of values not deemed meaningful, I would advise you to include that in your hypotheses (e.g. "AB > (APeabody, AAge), APeabody - AAge > -0.01, APeabody - AAge < 0.01").  
For more information, see the guidelines ('Guidelines_output_GORIC.html') and/or the benchmark tutorial on https://github.com/rebeccakuiper/Tutorials.
...
#library(gorica)
#
# Also load the bain library because it contains the data 'sesamesim'
# install this package first (once)
if (!require("bain")) install.packages("bain") # install this package first (once)
library(bain)
```
...
See https://doi.org/10.1080/10705511.2020.1836967, for more information.
...
```{r}
set.seed(100)
results3_eq <- goric(fit3, hypotheses = list(H1.3 = H1.3), 
                  #type = "gorica", # default
                  #comparison = "complement", # default
                  Heq = T) 
results3_eq
...
```{r}
set.seed(100)
goric(fit3, hypotheses = list(H1.3 =  H1.3), 
      standardized = T)$result[,3]
...
Now, use these when calculating the GORICA values and weights for H1.3 and its complement, via:
...
```{r}
set.seed(100)
results3_eq <- goric(est_3, VCOV = vcov_3, hypotheses = list(H1.3 =  H1.3),
                  Heq = T) 
results3_eq
```
...
set.seed(100)
goric(est_3, VCOV = vcov_3, 
      hypotheses = list(H1.3 =  H1.3)
      )$result[,3]

set.seed(100100)
goric(est_3, VCOV = vcov_3, 
      hypotheses = list(H1.3 =  H1.3))$result[,3]
...
# Exercise 4 (Optional): The sky is the limit

You can try out evaluating informative, theory-based hypotheses using other data sets; for example, one or more from the subfolder ‘data’ on the github repository https://github.com/rebeccakuiper/Tutorials or perhaps from https://openpsychometrics.org.

You can also look into 
<!-- applying the GORIC(A) and/or bain on  -->
other types of models or specific SEM models (e.g., a (RI-)CLPM or a mediation model).  
Examples files for the GORIC(A) can be found on https://github.com/rebeccakuiper/Tutorials. In '_README.md', you can find an overview of the (example and tutorial) files.

You can of course, when available, also use your own data.
