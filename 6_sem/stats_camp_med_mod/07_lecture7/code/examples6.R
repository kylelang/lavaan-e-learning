...
dat1 <- read.spss(paste0(dataDir, "three_wave_longitudinal_mediation.sav"),
                  to.data.frame = TRUE)
...
beta <- matrix(c(-0.5, 0, 0.5, -0.25, 0.01, 0.25, -0.4, -0.05, 0.4,
                 0.5, 0.5, 0, 0.25, 0.35, 0.01, 0.4, 0.34, 0.05),
               ncol = 2)

eta <- dat2 %*% beta
noise <- rmvnorm(500, c(0, 0), cov(eta))
covs <- eta + noise

colnames(covs) <- c("rad", "income")

dat3 <- data.frame(dat2, covs)

saveRDS(dat3, file = paste0(dataDir, "unicorn2.rds"))
...
out1 <- sem(mod1, data = dat3, se = "boot")
summary(out1)
...
out2 <- sem(mod2, data = dat3)
summary(out2)

chiDiff <- fitMeasures(out2)["chisq"] -
    fitMeasures(out1)["chisq"]

dfDiff <- fitMeasures(out2)["df"] -
    fitMeasures(out1)["df"]
...
att1 + att2 + att3 ~ income
conf1 + conf2 + conf3 ~ income
horn1 + horn2 + horn3 ~ income
...
out3 <- sem(mod3, data = dat3, se = "boot")
summary(out3)
...
out4 <- sem(mod4, data = dat3, se = "boot")
summary(out4)
...
out5 <- sem(mod5, data = dat3, se = "boot")
summary(out5)
