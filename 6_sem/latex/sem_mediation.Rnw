...
Structural equation modeling (SEM) simply combines path analysis and CFA.
\vb
\begin{itemize}
...
\vb
\begin{itemize}
\item When we fit a multiple-group SEM, we are modeling moderation by group.
\vc
\begin{itemize}
...
analysis and conditional process anlaysis.
\end{itemize}
...
\begin{frame}{Simplifying our Path Diagram}
  ...
  \textsc{Question:} If we don't care about directly estimating $c$, how can we
...
## Fit the model:
out1.1 <-
    sem(mod1.1, data = dat1, se = "boot", bootstrap = nBoot)
...
\begin{frame}[fragile, shrink = 5]{Example}
...
out1.2 <-
    sem(mod1.2, data = dat1, se = "boot", bootstrap = nBoot)
summary(out1.2)
...
out1.3 <-
    sem(mod1.3, data = dat1, se = "boot", bootstrap = nBoot)
summary(out1.3)
@
...
<<>>=
parameterEstimates(out1.3, boot.ci.type = bootType)[ , -c(1 : 3)]
@
...
out2.1 <-
    sem(mod2.1, data = dat1, se = "boot", bootstrap = nBoot)
summary(out2.1)
@
...
<<>>=
parameterEstimates(out2.1, boot.ci.type = bootType)[ , -c(1 : 3)]
@
...
out2.2 <-
    sem(mod2.2, data = dat1, se = "boot", bootstrap = nBoot)
summary(out2.2)
@
...
<<>>=
parameterEstimates(out2.2, bootstrap = bootType)[ , -c(1 : 3)]
@
...
out2.3 <-
    sem(mod2.3, data = dat1, se = "boot", bootstrap = nBoot)
summary(out2.3)
...
out3.1 <-
    sem(mod3.1, data = dat1, se = "boot", bootstrap = nBoot)
summary(out3.1)
@
...
<<>>=
parameterEstimates(out3.1, boot.ci.type = bootType)[ , -c(1 : 3)]
@
...
out4.1 <-
    sem(mod4.1, data = dat1, se = "boot", bootstrap = nBoot)
summary(out4.1)
@
...
<<>>=
parameterEstimates(out4.1, boot.ci.type = bootType)[ , -c(1 : 3)]
@
...
out1.2 <- sem(mod1.2, data = dat1, std.lv = TRUE,
              se = "boot", bootstrap = 2500)
summary(out1.2)
parameterEstimates(out1.2, boot.ci.type = "bca.simple")[-c(1 : 3)]
@
