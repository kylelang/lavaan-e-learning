...
lavaanPlot(model = fit_CFA, node_options = list(shape = "box", fontname = "Helvetica"), 
           ...
           #stand = T, # if you want standardized estimates
           covs = T)
```
...
# Based on the AIC:
fit_EFA_AIC <- anova(fit_1EFA, fit_2EFA)$AIC
#names(fit_EFA_AIC) <- c("2EFA", "1EFA") # Note 2EFA comes first! # names not needed
#fit_EFA_AIC
#if (!require("restriktor")) install.packages("restriktor", dependencies = TRUE)
library(restriktor) # ?calc_ICweights 
AIC_weights_EFA <- calc_ICweights(fit_EFA_AIC, hypo_names = c("2EFA", "1EFA"))
AIC_weights_EFA$IC_weights
AIC_weights_EFA$ratio_IC_weights[1,2]
# Note: More information in IC weights can be found in the Model Selection lecture.
```

The Chi-square difference test is significant, implying that the null (stating that two factors are one and the same) is rejected. Additionally, the AIC and BIC for the 2-factor model is lower than for the 1-factor model. Moreover, the 2-factor model is infinite more supported than the 1-factor model.  
...
# In case you want to quantify the relative support (so, how much better).
# Based on the AIC:
fit_EFA_AIC <- compare_performance(fit_1EFA, fit_2EFA)$AIC
names(fit_EFA_AIC) <- c("1EFA", "2EFA") # names not needed
#fit_EFA_AIC
library(restriktor) # ?calc_ICweights
AIC_weights_EFA <- calc_ICweights(fit_EFA_AIC, hypo_names = c("1EFA", "2EFA"))
AIC_weights_EFA$IC_weights
AIC_weights_EFA$ratio_IC_weights[1,2]
# Note: More information in IC weights can be found in the Model Selection lecture.
```
...
# In case you want to quantify the relative support (so, how much better).
# Based on the AIC:
fit_AIC <- anova(fit_1CFA, fit_2CFA)$AIC
names(fit_AIC) <- c("2CFA", "1CFA") # Note 2CFA comes first! # names not needed
#fit_AIC
#library(restriktor)
AIC_weights <- calc_ICweights(fit_AIC, hypo_names = c("2CFA", "1CFA"))
AIC_weights$IC_weights
AIC_weights$ratio_IC_weights[1,2]
# Note: More information in IC weights can be found in the Model Selection lecture.
```
The Chi-square difference test is significant, implying that the null (stating that the two factors are one and the same) is rejected. Additionally, the AIC and BIC for the 2-factor model is lower than for the 1-factor model. Moreover, the 2-factor model is infinite more supported than the 1-factor model.  
Hence, we prefer the 2-factor model.
