...
\begin{frame}{Estimating Products of Latent Variables}
  ...
  \item Latent moderated structural equations \citep[LMS;][]{kleinEtAl:1997, 
    ...
    \vc
  \item Structural-after-measurement (SAM) estimation \citep{rosseelLoh:2022}.
    \vc
  \item Bayesian SEM. 
  \end{enumerate}
  \va
  Only the SAM approach is currently implemented in \pkg{lavaan}.
  \vc
  \begin{itemize}
  \item The current implementation cannot probably handle latent variable interactions.
  \end{itemize}
