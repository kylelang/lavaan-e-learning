...
progress ~ success + disillusion + success:success + success:disillusion
'

samOut <- outlook %>% 
  select(-success, -disillusion) %>%
  sam(samMod, data = ., std.lv = TRUE, meanstructure = TRUE, se = "bootstrap", bootstrap = 10)

?lavOptions
summary(samOut, ci = TRUE)
