...
## Use a dplyr pipeline to create the product term and estimate the model:
lavOut <- sem(mod1, data = dDat)
@
...
\begin{frame}[shrink = 5]{Latent Variable Interactions}
  ...
  our job is easy:
  \begin{enumerate}
    \item Construct the product term of the observed focal and moderator
      variables
    \item Use the observed focal, moderator, and interaction variables to
      predict the latent DV
  \end{enumerate}
  \va If we want to model moderation when at least on of the predictors is
  latent, things get more difficult.
  \begin{itemize}
  \item If the moderator is observed and discrete, we can use multiple group
    modeling
  \item If the moderator is continuous and/or latent, then we need fancier
    methods
  \end{itemize}
  \va
...
\begin{frame}{Computing Interaction Indicators}
  ...
  those terms as indicators of the interaction construct.  
  \vb
  \begin{itemize}
  ...
  \item Collinearity among the interaction indicators and the raw items can
    ...
    \vb
  \item From a modeling perspective, we'd like to interpret out final model
    holistically
  \end{itemize}
  \va
...
\begin{frame}[allowframebreaks]{Orthogonalization}
  ...
  Say we want to estimate the moderated effect of $Z$ on the $X \rightarrow Y$
  effect, where $X$, $Y$, and $Z$ are latent variables indicated by $\{x_1, x_2,
...
partSummary(out3, 5, 1:14)
@ 

\pagebreak

<<>>=
partSummary(out3, 5, c(1, 2, 15:24))
@ 

\pagebreak

<<>>=
partSummary(out3, 6:7, -(14:39))
@ 
...
\begin{frame}[allowframebreaks]{Double Mean Centering}
  ...
  We could also specify the interaction factor using double mean centering. 
  \vb
  \begin{enumerate}
...
partSummary(out4, 5, 1:14)
@ 

\pagebreak

<<>>=
partSummary(out4, 5, c(1, 2, 15:24))
@ 

\pagebreak

<<>>=
partSummary(out4, 6:7, -(10:35))
@ 
...
\begin{frame}{Estimating Products of Latent Variables}
  ...
  \item Latent moderated structural equations \citep[LMS;][]{kleinEtAl:1997, 
    ...
    \vc
  \item Structural-after-measurement (SAM) estimation \citep{rosseelLoh:inPress}.
    \vc
  \item Bayesian SEM. 
  \end{enumerate}
  \va
  None of the above is currently implemented in \pkg{lavaan}.
  \vc
  \begin{itemize}
  \item The SAM approach should be available soon.
  \end{itemize}
...
\begin{frame}{Estimating Products of Latent Variables}
  ...
  We can directly estimate the interaction between two latent variables with the
  \emph{latent moderated structural equations} (LMS) method.
  \va
  \begin{itemize}
  \item Introduced by \citet{kleinEtAl:1997} and formalized by
    \citet{kleinMoosbrugger:2000} 
    \vb
  \item Currently only available in Mplus (via the \src{Xwith} command).
    \vb
...
\begin{frame}{Multiple Moderation}
  ...
  \end{align*}\\
  \va
  We can easily fit this model in any regression (or path modeling) software.
  \vb
  \begin{itemize}
...
\begin{frame}{Moderated Moderation}
  ...
  \begin{align*}
    Y =& ~ \alpha + \beta_1X + \beta_2Z + \beta_3W + \beta_4XZ + \beta_5XW + \beta_6ZW + \beta_7XZW + e
  \end{align*}\\
  \vb
  Which can be restructured into:
  \begin{align*}
    Y =& ~ \alpha + (\beta_1 + \beta_4Z + \beta_5W + \beta_7ZW)X + \beta_2Z + \beta_3W + \beta_6ZW + e\\[6pt]
...
sem(mod, data = socSup) %>% partSummary(5:7)
@ 
...
compareFit(out0, out1, out2) %>% partSummary(1:4, -(2:3))
@
...
partSummary(out3, c(6, 8, 9, 12, 14, 15), c(1:9, 20, 22:31, 42))
@ 
