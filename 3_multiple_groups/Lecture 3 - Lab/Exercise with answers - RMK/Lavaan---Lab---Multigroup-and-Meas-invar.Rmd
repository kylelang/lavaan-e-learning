...
# Note: I ask here for the standardized estimates
...
# sw ~~ sw
# 
# # means and (co)variances of the predictors
# overt ~ 1
# covert ~ 1
# overt ~~ overt
# covert ~~ covert
...
<!-- # Reference-group scaling -->
<!-- • Factor variance constrained at 1. -->
<!-- • Factor mean constrained at 0.* -->
<!-- * In a multi-group model (discussed in another lecture), the mean of the -->
<!-- (by default) first group is set to 0 (hence, the name ’reference-group’). -->
...
<!-- # Marker-variable method (default) -->
...
<!-- #F ~~ NA*F -->
<!-- " -->
<!-- # NOTE: nu alleen 1e factor een lading van 1!!! In Default lavaan in alle factoren.... -->
<!-- # Dat default model dan vergelijken met beide latent variable varianties gelijk aan 1 zetten?? -->
<!-- # Fit: -->
<!-- fit_MG1CFA <- cfa(model.1CFA, data=data_PGD, -->
<!--                 group = 'Kin2', -->
<!--                 missing='fiml', fixed.x=F) # Specify FIML  -->
...
#lavInspect(fit_MG1CFA_ci) 
# npar = number of free parameters = 30 - 0 = 30, namely
# 30 parameters & 0 equality restrictions
```

<!-- NB npar = number of free parameters. -->
<!-- fitmeasures(fit_MG1CFA_ci, "npar") # 30 -->
...
#lavInspect(fit_MG1CFA_wi) 
# npar = number of free parameters = 30-4 = 26, namely
# 30 parameters & 4 equality restrictions: on the 4 free factor loadings  ('lambda')
```

<!-- NB npar = number of free parameters. -->
<!-- fitmeasures(fit_MG1CFA_wi, "npar") # 26 -->

*Note on constraints in output:*  
At the top of of the output, lavaan tells you with "1 == 16" that it restricts the 1st and 16th to-be-estimated parameters to be the same; and likewise for the other parameters.  
If you look at the output, you see that 1 (so, the 1st parameter of the to-be-estimated parameters) is the factor loading (lambda) of item b2pss2 of the group 'partners':
````{verbatim}
$partner
$partner$lambda
       F
b2pss2 1
````
Likewise, you find that 16 (so, the 16th parameter of the to-be-estimated parameters) is the factor loading (lambda) of item b2pss2 of the group 'else':
````{verbatim}
$`else`
$`else`$lambda
        F
b2pss2 16
````
So, lavaan restricts the factor loading (lambda) of item b2pss2 of the group 'partners' to be equal to the factor loading (lambda) of b2pss2 of the 'else' group.  
...
#lavInspect(fit_MG1CFA_si) 
# npar = number of free parameters = 31-9 = 22, namely
# 31 parameters (now, also estimating alpha of the second group) & 9 equality restrictions: on the 4 free factor loadings ('lambda') and on the 5 intercepts ('nu')
```

<!-- NB npar = number of free parameters. -->
<!-- fitmeasures(fit_MG1CFA_si, "npar") # 22 -->

*Note on number of parameters:*  
In each group, 5 intercepts ('nu') are estimated: if we would constrain these, we would be saying that the means across the two groups are equal.
However, what we want to say is that the differences in observed means are completely explained by mean differences on the latent variables.
Therefore, we constrain the intercepts across the groups ('nu'), and free the latent mean(s) in one of the groups ('alpha'): these represent the latent mean differences. Notably, when you have multiple groups, the latent mean(s) are freed (or you should free them) for all-but-one groups.  
Hence, the total number of parameters to be estimated here is 31. 
<!-- Notably, at the same time 9 parameters are restricted, so there are actually 31-9=22 unique parameters estimated. # TO DO dit verward ms alleen meer ivm de Df die niet 22 is! -->
...
```
<!-- Note on Df diff:  -->
<!-- ci vs wi: 4 because of restricting the 4 free factor loadings. -->
<!-- wi vs si: 4 because of (additionally) restricting the 5 intercepts ('nu') but also freeing the latent mean ('alpha'), so 5-1=4. -->
...
Alternatively or additionally, one could look at the AIC and/or BIC values. Since the AIC and BIC are the lowest for 'MG1CFA_si', we also conclude from this that there is strong invariance. We could even quantify the relative support:
```{r, message=F, warning=F}
# In case you want to quantify the relative support (so, how much better).
# Based on the AIC:
fit_AIC <- lavTestLRT(fit_MG1CFA_ci, fit_MG1CFA_wi, fit_MG1CFA_si)$AIC
# or:
#fit_AIC <- anova(fit_MG1CFA_ci, fit_MG1CFA_wi, fit_MG1CFA_si)$AIC
#names(fit_AIC) <- c("ci", "wi", "si") # not needed
#fit_AIC
#if (!require("restriktor")) install.packages("restriktor", dependencies = TRUE)
library(restriktor) # ?calc_ICweights 
AIC_weights <- calc_ICweights(fit_AIC, hypo_names = c("ci", "wi", "si"))
#AIC_weights
AIC_weights$IC_weights # 'si' (3rd model) is best
AIC_weights$ratio_IC_weights[3,1:2]
# Note: More information in IC weights can be found in the Model Selection lecture.
```
<!-- The weak invariance (equal factor loadings) model is about -->
<!-- `r round(as.numeric(AIC_weights$ratio_IC_weights[2,1]), 2)` -->
<!-- times more supported than the configural model. -->
<!-- Additionally, the  -->
The strong invariance model is about
`r round(as.numeric(AIC_weights$ratio_IC_weights[3,1]), 2)`
times more supported than the configural model, and about
`r round(as.numeric(AIC_weights$ratio_IC_weights[3,2]), 2)`
times more than the weak invariance (equal factor loadings) model.
...
# npar = number of free parameters = 31-14 = 17, namely
# 31 parameters (now, also estimating alpha of the second group) & 14 equality restrictions: on the 4 free factor loadings ('lambda'), on the 5 intercepts ('nu'), and on the 5 residual variances ('theta')
```

<!-- NB npar = number of free parameters. -->
<!-- fitmeasures(fit_MG1CFA_si_res, "npar") # 17 -->

<!-- Note on Df diff:  -->
<!-- si vs si_res: 5 because of (additionally) restricting the 5 residual variances. -->
...
Van de Schoot et al. (2012) note that with the lack of strict invariance, groups can still be compared on the latent construct(s).  
Reference: Van de Schoot, R., Lugtig, P., & Hox, J. (2012). A checklist for testing measurement invariance. European Journal of Developmental Psychology, 9(4), 486–492.
...
<!-- C:\WERK\Ow - S65 - e-learning lavaan\0 - Materiaal andere cursussen\SEM Ellen -->

<!-- Lecture 2 slides: mn slide 26 -->

<!-- Model 2: Equal factor loadings -->

<!-- not significant: -->
<!-- Hence, we do not have to reject the null model (Model 2 with equal factor loadings) in favor of the alternative model (Model 1 without constraints). -->
<!-- Put differently: We can assume there are equal factor loadings. -->

<!-- Model 3: Equal intercepts and factor loadings -->
<!-- In each group, 12 intercepts were estimated: if we would constrain these, we would be saying that the means across the two groups are equal. -->
<!-- However, what we want to say is that the differences in observed means are completely explained by mean differences on the latent variables. -->
<!-- Therefore, we constrain the intercepts across the groups, and free the latent means in one of the groups: these represent the latent mean differences. -->
<!-- Hence we have: 12 – 4 = 8 parameters less in Model 3 compared to Model 2. -->

<!-- Model 3: Strong Factorial Invariance -->

<!-- Nesting of model 2 and model 3 -->
<!-- At first it may be confusing that in model 3 we constrain the intercepts AND free latent means in the second group: How does this lead to nested models? -->
<!-- It helps to write down the modeled mean differences between the two groups in each model. -->
<!-- In model 2 we have: -->
<!-- This requires 12 additional parameters (in addition to the 12 mean parameters in group 1). -->
<!-- In model 3 we have: -->
<!-- This requires only 4 additional parameters (in addition to the 12 mean parameters in group 1). -->

<!-- significant: -->
<!-- Hence, we cannot impose the constraint of equal intercepts. -->
<!-- There is no strong factorial invariance! -->
<!-- To detect sources of bias, we look at the modification indices: they indicate how much the model would improve if we free this parameter. -->
