...
\begin{frame}[fragile]{CFA Example}
  ...
  It is very easy to apply FIML estimation to latent variable models in 
  \pkg{lavaan}.
...
est <- c(coef(compOut)[what],
         ...
         coef(fimlOut)[what],
         coef(fimlOut2)[what])
names(est) <- c("Complete Data", "Deletion", "MI", "Naive FIML", "FIML w/ Auxiliaries")
...
#summary(fimlOut, fmi = TRUE)$pe[names(coef(fimlOut)) %in% what]

#partSummary(fimlOut2, 8, 1:4, fmi = TRUE)
#partSummary(miOut, 8, fmi = TRUE)
@
