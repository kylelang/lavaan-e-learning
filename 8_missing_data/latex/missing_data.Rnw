...
\begin{frame}[fragile]{FIML Example}
  ...
  To use FIML in \pkg{lavaan}, we only nee to set the \src{missing = ¨fiml¨} 
  argument.
  \begin{itemize}
...
\begin{frame}[fragile]{Saturated Correlates Example}
  ...
  We can use the \src{lavaan.auxiliary()} function (or one of its wrappers) from 
  \pkg{semTools} to streamline the analysis.
