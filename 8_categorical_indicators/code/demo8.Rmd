...
for continous indicator models). 
...
1. We estimate a restricted model wherein the threhsolds are constrained to 
equality across groups.
1. We test the tenability of the constraint by comparing to the configurally 
invariant model.

We must establish threshold invariance before considering weak or strong invariance 
because weak and strong invariance both concern paths linking the latent response 
variables to the latent constructs. We must first demonstrate, therefore, that 
the latent response variables are defined equivalently in all groups.

---

We can make our lives much easier by using the `measEa.syntax()` function from 
the **semTools** package to programatically generate the **lavaan** model syntax
for our models.
...
(i.e., by wrapping it with `as.character()`) to get the `cfa()` function to recognize 
it as model syntax.
...
As expected, the thresholds are equal across groups. Constraining the thresholds
helps identify the model, so we can freely estimate the item intercepts and the 
scales of the latent response variables in the second and third groups.
...
the most refined $\Delta \chi^2$ test.
   ...
   to account for the scaled and shifted fit statistics.
...
We conclude by equating the item intercepts accross groups to test strong/metric
invariance.
...
There are two common caveats to the above statement.
...
1. The WLS and DLWS estimators cannot handle missing data (i.e., there is no WLS 
analog to FIML), so you must treat any missing prior to model estimation. With 
FIML off the table, you should use multiple imputation (MI) unless you have 
...
imputation will suffice).
   ...
   covered in Module 6 make the process of conducting these analyses with multiply 
   imputed data relatively painless.
