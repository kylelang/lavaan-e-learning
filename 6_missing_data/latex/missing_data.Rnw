...
\newcommand{\betab}[0]{\boldsymbol{\betaup}}
...
source(here::here(codeDir, "sim_missing", "code", "simMissingness.R"))
source(here::here(codeDir, "ml_functions.R"))
...
\begin{frame}{What are Missing Data?}
  ...
  \vc
  \begin{itemize}
  \item The missing cells correspond to true population values, but we haven't observed those values.
  \end{itemize}
  \vb
  \pause
...
\begin{frame}{A Little Notation}
  ...
  \begin{align*}
    Y &\coloneq \text{An $N \times P$ Matrix of Arbitrary Data}\\[8pt]
    Y_{mis} &\coloneq \text{The \emph{missing} part of $Y$}\\[8pt]
    Y_{obs} &\coloneq \text{The \emph{observed} part of $Y$}\\[8pt]
    R &\coloneq \text{An $N \times P$ response matrix}\\[8pt]
    M &\coloneq \text{An $N \times P$ missingness matrix}
  \end{align*}

  The $R$ and $M$ matrices are complementary.
  \begin{itemize}
  \item $r_{np} = 1$ means $y_{np}$ is observed; $m_{np} = 1$ means $y_{np}$ is missing.
  \item $r_{np} = 0$ means $y_{np}$ is missing; $m_{np} = 0$ means $y_{np}$ is observed.
...
## Read in some data:
bfi0 <- readRDS(here::here("data", "bfi_datasets.rds"))
...
## Estimate the model:
naiveOut <- cfa(cfaMod, data = bfi, std.lv = TRUE)
@ 
...
\begin{frame}{Default Approach}
  ...
  Like most software packages, \pkg{lavaan} will default to \emph{complete case analysis} when asked to analyze
  incomplete data.
  ...
  \item In the absence of user input, this is a sensible option.
    \vc
  \item That doesn't mean you should actually use deletion to treat the missing data in your analysis.
  \end{itemize}
...
\begin{frame}{Missing Data Mechanisms}
  ...
  \item $P(R | Y_{mis}, Y_{obs}) = P(R | Y_{obs})$
    \vc
  \item Missingness is related to only the \emph{observed} parts of study variables.
  \end{itemize}
  \vb
  %\begin{align*}
  ...
  \item $P(R | Y_{mis}, Y_{obs}) \neq P(R | Y_{obs})$
    \vc
  \item Missingness is related to the \emph{unobserved} parts of study variables.
  \end{itemize}
  %\begin{align*}
  %  P(R | Y_{mis}, Y_{obs}) \neq P(R | Y_{obs})
  %\end{align*}
...
\begin{frame}[fragile]{Effects of Deletion}
  ...
  \vc
  \begin{itemize}
  \item The statistics upon which we base our analyses generally summarize these distributions.
    \vc
  \item Problems with the distributions show up as bias in the results of our analyses.
  \end{itemize}
  
<<>>=
diabetes1 <- diabetes2 <-
  readRDS(here::here("data", "diabetes.rds"))
...
\begin{frame}[allowframebreaks]{Good Missing Data Methods}
  ...
  \item Replace the missing values with $M$ plausible estimates
    \vc
    \begin{itemize}
    \item Essentially, a repeated application of stochastic, regression-based prediction to synthesize replacements for
      the missing data.
      \vc
  ...
  \vc
  \begin{itemize}
  \item Adjust the objective function to only consider the observed parts of the data
    \vc
    \begin{itemize}
    ...
    \item The predictors of nonresponse must be included in the model, somehow
      \vc
    \item FIML is only available for certain types of models
      \vc
    \item In linear regression models, FIML cannot treat missing data on predictors (if the predictors are taken as
      fixed)
    \end{itemize}
  \end{itemize}
...
\begin{frame}[allowframebreaks, fragile]{Good Missing Data Methods}
...
\begin{itemize}
\item MI produces unbiased estimates when data are MAR.
\end{itemize}
...
\begin{frame}[fragile, allowframebreaks]{Good Missing Data Methods}
...
\begin{itemize}
\item MI cannot correct bias when data are MNAR.
\end{itemize}
...
\begin{frame}{Prediction Example}
  ...
  To fix ideas, let's consider the following model:
  \begin{align*}
...
Training this model on the first $N = 400$ observations from the \emph{diabetes} data produces the following fitted
  model:
\begin{align*}
...
\begin{frame}{*Levels of Uncertainty Modeling}
  ...
  \citet{vanBuuren:2018} classifies different imputation methods:
  \vb
  \begin{enumerate}
  ...
  \item Prediction + Noise + Model Error
    ...
    \item Required for proper imputations in the sense of \citet{rubin:1987}.
    \end{itemize}
  \end{enumerate}
...
simRes <- readRDS(here::here(dataDir, "simResMat.rds"))
...
\item Both single imputation methods inflate Type I error rates.
  \vc
\item MI provides unbiased estimates and accurate Type I error rates.
\end{itemize}
...
\begin{frame}[fragile]{Flavors of MI}
  ...
  \begin{itemize}
  \item The specifics of this regression imputation are important.
  \end{itemize}
  \vb
  \pause
...
<<echo = FALSE, results = "asis">>=
simRes2 <- readRDS(here::here(dataDir, "simResMat2.rds"))
...
\begin{itemize}
\item Type I error rates for PN-Type MI improve on single stochastic regression imputation, but they're still too high.
\end{itemize}
...
\begin{frame}{Proper MI}
  ...
  The problems on the previous slide arise from using the same regression coefficients to create each of the $M$
  imputations.
  \vc
  \begin{itemize}
  \item Implies that you're using the ``correct'' coefficients.
    ...
    \end{itemize}
    \vb
  \item Proper MI also models uncertainty in the regression coefficients used to create the imputations.
    \vc
    \begin{itemize}
    \item A different set of of coefficients is randomly sampled (using Bayesian simulation) to create each of the $M$
      imputations.
      \vc
    \item The tricky part about implemented MI is deriving the distributions from which to sample these coefficients.
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Setting Up Proper MI}
  ...
  \end{align*}
  To fully account for model uncertainty, we need to randomly sample both $\beta$ and $var(\varepsilon) = \sigma^2$.
  \begin{itemize}
  \item \rmsc{Question:} Why do we only sample $\sigma^2$ and not $\varepsilon$?
  \end{itemize}
  \pause
  \va
  For an imputation model with a normally distributed outcome and uninformative priors, we need to specify two
  distributions:
  \begin{enumerate}
...
\begin{frame}{Marginal Distribution of $\sigma^2$}
  ...
  \end{itemize}
  \begin{align}
    \sigma^2 &\sim Inv\text{-}\chi^2 \left(N - P, MSE \right) \label{sigma2PosteriorEq}\\
...
\begin{frame}{Conditional Distribution of $\beta$}
  ...
  \end{itemize}
  \begin{align}
    \beta \sim MVN \left( \hat{\beta}_{ls}, ~ \sigma^2 (\mathbf{X}^T \mathbf{X})^{-1} \right) \label{betaPosteriorEq}
  \end{align}
  \vx{-12}
  \begin{itemize}
...
\begin{frame}{PPD of the Missing Data}
  ...
  Once we've sampled our imputation model parameters, we can construct the posterior predictive distribution of the
  missing data.
  \vc
  \begin{itemize}
  \item This is the distribution from which we sample replacements for the missing values (i.e., imputations).
    \vc
  \item In practice, we directly compute the imputations based on the simulated imputation model parameters.
  \end{itemize}
  \begin{align}
    Y_{imp} &= \mathbf{X}_{mis}\tilde{\beta} + \tilde{\varepsilon} \label{impPosteriorEq}\\
    \varepsilon &\sim N \left( 0, \widetilde{\sigma^2} \right) \notag
  \end{align}
...
\begin{frame}{General Steps for Basic MI}
  ...
  \vb
  \begin{enumerate}
  \item Find the least squares estimates of $\beta$, $\hat{\beta}_{ls}$, by regressing the observed portion of $Y$ onto
    the analogous rows of $\mathbf{X}$.
    \vb
  \item Use $\hat{\beta}_{ls}$ to parameterize the posterior distribution of $\sigma^2$, given by Equation
    \ref{sigma2PosteriorEq}, and draw $M$ samples of $\sigma^2$ from this distribution.
    \vb
  \item For each of the $\sigma^2_m$, sample a corresponding value of $\beta$ from Equation \ref{betaPosteriorEq}.
    \vb
  \item Plug the $M$ samples of $\beta$ and $\sigma^2$ into Equation \ref{impPosteriorEq} to create the $M$ imputations.
  \end{enumerate}
...
\begin{column}{0.5\textwidth}
  ...
  \only<3>{
    ...
    \end{itemize}
    \vb
    Generate imputations as in Stochastic Regression Imputation:
    \begin{align*}
      Y_{imp} &= \hat{Y}_{mis} + \varepsilon\\
      \varepsilon &\sim N(0, \Sexpr{round(sigmas[1], 3)})
    \end{align*}
  }
  ...
  \only<5>{
    ...
    \begin{align*}
      Y_{imp} &= \hat{Y}_{mis} + \varepsilon\\
      \varepsilon &\sim N(0, \Sexpr{round(sigmas[2], 3)})
    \end{align*}
  }
  ...
  \only<7>{
    ...
    \begin{align*}
      Y_{imp} &= \hat{Y}_{mis} + \varepsilon\\
      \varepsilon &\sim N(0, \Sexpr{round(sigmas[3], 3)})
    \end{align*}
  }
...
\begin{frame}{Extending the Basic MI Algorithm}

  The basic MI algorithm described above can be extended in two important ways.
  \vb
  \begin{enumerate}
  \item Relaxing the normality assumption
    \vc
    \begin{itemize}
    \item Model the missing data with a non-normal distribution
      \vc
    \item Generate imputations from matched, observed cases
      \vc
    \item Generate imputations from non-parametric algorithms
    \end{itemize}
    \vb
  \item Imputing multivariate data
    \vc
    \begin{itemize}
    \item Joint modeling
      \begin{itemize}
        \item \citet{rubin:1978, rubin:1987, schafer:1997, honakerKing:2010}
      \end{itemize}
      \vc
    \item Fully conditional specification 
      \begin{itemize}
        \item \citet{raghunathanEtAl:2001, vanBuurenEtAl:2006}
      \end{itemize}
    \end{itemize}
  \end{enumerate}
...
\begin{frame}{Doing MI-Based Analysis}
  ...
  \item The pooling phase
    \begin{itemize}
    \item Combine the $M$ sets of parameter estimates and standard errors from Step \ref{aStep} into a single set of MI
      estimates.
    \item Use these pooled parameter estimates and standard errors for inference.
    \end{itemize}
  \end{enumerate}
...
\begin{frame}{Pooling MI Estimates}
  ...
  \vb
  \begin{itemize}
  \item The MI point estimate of some interesting quantity, $Q^*$, is simply the mean of the $M$ estimates,
    $\{\hat{Q}_m\}$:
    \begin{align*}
...
\begin{frame}{Pooling MI Estimates}
  ...
  \vb
  \begin{itemize}
  \item A weighted sum of the \emph{within-imputation} variance, $W$, and the \emph{between-imputation} variance, $B$.
    \begin{align*}
...
\begin{frame}{Fraction of Missing Information}
  ...
  The \emph{fraction of missing information} (FMI) is a crucial effect size measure for missing data problems.
  ...
  \vc
  \begin{itemize}
  \item We should report the FMI for an estimated parameter along with other statistics (e.g., t-tests, p-values, effect
    sizes, etc.).
  \end{itemize}
...
\begin{frame}[fragile]{Example: Analysis \& Pooling}
  ...
  We can use the \src{lavaan.mi()} function (or one of its wrappers) from the \pkg{lavaan.mi} package to analyze the
  multiply imputed datasets and pool results.
...
<<echo = FALSE>>=
partSummary(miOut, 1:4, fmi = TRUE, header = FALSE)
@ 
...
<<echo = FALSE>>=
partSummary(miOut, 7, fmi = TRUE, header = FALSE, drops = c("df", "riv"))
@ 
...
<<echo = FALSE>>=
partSummary(miOut, 8, fmi = TRUE, header = FALSE, drops = c("df", "riv"))
@ 
...
<<echo = FALSE>>=
partSummary(miOut, 9, fmi = TRUE, header = FALSE, drops = c("df", "riv"))
@ 
...
\begin{frame}{Model Fit, Model Comparisons, MV Testing}
  
  We cannot simply average the $\chi^2$ statistics as we do the parameter estimates.
  \vc
  \begin{itemize}
  \item We need some fancy processing to get an appropriate statistic.
    \end{itemize}
  \vb
  Three different formulations have been proposed:
  \vc
  \begin{itemize}
    \item $D_1$: \citet{d1_paper}
      \begin{itemize}
      \item Multivariate Wald-test
      \end{itemize}
      \vc
    \item $D_2$: \citet{d2_paper}
      \begin{itemize}
      \item Direct aggregate of the $M$ test statistics or p-values
      \end{itemize}
      \vc
    \item $D_3$: \citet{d3_paper}
      \begin{itemize}
      \item Likelihood ratio statistic
      \end{itemize}
  \end{itemize}
  \vb
  See \citet[][\href{https://stefvanbuuren.name/fimd/sec-multiparameter.html}{\textcolor{blue}{Section 5.3}}]{vanBuuren:2018} for more details.
...
\begin{frame}[fragile, allowframebreaks]{Model Fit}
  ...
  When we submit the \src{lavaan.mi} object created by \src{cfa.mi()} to the \src{fitMeasures()} function, we can
  specify the type of pooling rule.

<<>>=
what <- c("chisq", "df", "pvalue", "cfi",
          "rmsea", "rmsea.ci.lower", "rmsea.ci.upper")
fitMeasures(miOut, fit.measures = what, pool.method = "D3") |>
  round(3)
@
...
<<>>=
fitMeasures(miOut, fit.measures = what, pool.method = "D2") |>
  round(3)
@ 
...
anova(miOut2, miOut, pool.method = "D3")
@

\pagebreak

<<>>=
anova(miOut2, miOut, pool.method = "D2")
@ 
...
\begin{frame}[fragile]{Multivariate Testing}
...
\begin{frame}[fragile]{Multivariate Testing}
  ...
  Now, we test the constraints via multivariate Wald tests.

<<>>=
lavTestWald.mi(miOut3, constraints = cons, pool.method = "D1")
lavTestWald.mi(miOut3, constraints = cons, pool.method = "D2")
@ 
...
\begin{frame}{FIML Intuition}
  ...
  \vc
  \begin{itemize}
  \item FIML partitions the missing information out of the likelihood function so that the model is only estimated from
...
\begin{frame}[allowframebreaks]{Maximum Likelihood Estimation}
  ...
  \vb
  \begin{itemize}
  \item The \emph{likelihood} function is just a probability density (or mass) function with the data treated as fixed
    and the parameters treated as random variables.
    \vb
  \item Having such a framework allows us to ask: ``Given that I've observed these data values, what parameter values
    most probably describe these data?''
  \end{itemize}
  ...
  \vb
  \begin{itemize}
  \item In practice, we nearly always work with the natural logarithm of the likelihood function (i.e., the
    \emph{loglikelihood}).
  \end{itemize}
...
\begin{frame}[fragile, allowframebreaks]{ML Intuition}
  ...
  \vc
  \begin{itemize}
  \item We assume these data come from a normal distribution with a known variance of $\sigma^2 = 1$.  
    \vc
...
\begin{frame}{Likelihoods}
  ...
  If we plug estimated parameters into Equation \ref{margPdf}, we get the probability of observing $Y_n$ given
  ...
  Applying Equation \ref{estMargPdf} to all $N$ observations and multiplying the results produces a \emph{likelihood}:
  \begin{align*}
    L \left( \hat{\mu}, \hat{\sigma}^2 \right) = 
...
\begin{frame}{Likelihoods}
  ...
  We generally want to work with the natural logarithm of Equation \ref{estMargPdf}. Doing so gives the
  \emph{loglikelihood}:
  \begin{align*}
  \mathcal{L} \left( \hat{\mu}, \hat{\sigma}^2 \right) &= 
  ...
  ML tries to find the values of $\hat{\mu}$ and $\hat{\sigma}^2$ that maximize $\mathcal{L} \left( \hat{\mu},
  \hat{\sigma}^2 \right)$.
  \vc
  \begin{itemize}
  \item Find the values of $\hat{\mu}$ and $\hat{\sigma}^2$ that are \emph{most likely}, given the observed values of
    $Y$.
  \end{itemize}
...
\begin{frame}{Likelihoods}
  ...
  If we plug our estimated parameters into Equation \ref{olsPdf}, we get the probability of observing $Y_n$ given
  $\hat{Y}_n = \hat{\beta}_0 + \hat{\beta}_1X_n$ and $\hat{\sigma}^2$.
  \begin{align}
...
\begin{frame}{Likelihoods}
  ...
  \begin{align*}
  \mathcal{L} \left( \hat{\beta}_0, \hat{\beta}_1, \hat{\sigma}^2 \right) &= 
...
## Extract the predicted values and the estimated residual 
## standard error:
yHat <- predict(out1)
...
\begin{frame}{Likelihoods}
  ...
  Plugging our estimates into the logliklihood equation gives:
  \begin{align*}
  \mathcal{L} \left( \hat{\betab}, \hat{\sigma}^2 \right) =& 
    %\ln \prod_{n = 1}^N P \left( Y_n|X_n, \hat{\betab}, \hat{\sigma}^2 \right)\\[6pt] 
    -\Sexpr{nrow(diabetes) / 2} \times \ln \left( 2\pi \right) - \Sexpr{nrow(diabetes)} \times \ln \left( \Sexpr{round(s, 2)} \right)\\
    &-\frac{\sum_{n = 1}^{\Sexpr{nrow(diabetes)}} \left( Y_n - \Sexpr{round(coef(out1)[1], 2)} - \Sexpr{round(coef(out1)[2], 2)} X_n^{bp} - \Sexpr{round(coef(out1)[3], 2)} X_n^{glu} - \Sexpr{round(coef(out1)[4], 2)} X_n^{bmi}\right)^2}{\Sexpr{round(2 * s^2, 2)}}\\[6pt]
    =& \Sexpr{round(logLik(out1), 2)}
  \end{align*}
...
\begin{frame}{Likelihood Function Visualized}
  
  \begin{columns}[onlytextwidth]
    \begin{column}{0.5\textwidth}
      
      Here, we see the conditional loglikelhood function for $\beta_{glu}$.
      \vc
      \begin{itemize}
        \item What loglikelihoods do we get by varying the value of $\hat{\beta}_{glu}$ while keep all other parameters
...
x <- seq(beta[3] - margin, beta[3] + margin, length.out = 500)

llY <- rep(NA, length(x))
for(i in 1:length(x)) {
    yHat <- with(diabetes, beta[1] + beta[2] * bp + x[i] * glu + beta[4] * bmi)
    llY[i] <- dnorm(diabetes$ldl, yHat, s, log = TRUE) |> sum()
}

tmp <- data.frame(x = beta[3], y0 = min(llY), y1 = logLik(out1)[1])
gg0(x, llY, FALSE) + 
    geom_line() + 
...
\begin{frame}{Steps of ML}
  ...
  \begin{enumerate}
  \item Choose a probability distribution, $f(Y|\theta)$, to describe the distribution of the data, $Y$, given the
    parameters, $\theta$.
    \vc
  \item Choose some estimate of $\theta$, $\hat{\theta}^{(i)}$.
    \vc
  \item Compute each row's contribution to the loglikelihood function by evaluating: $\ln
    \left[f\left(Y_n|\hat{\theta}^{(i)}\right)\right]$. \label{rowContrib}
    \vc
  \item Sum the individual loglikelihood contributions from Step \ref{rowContrib} to find the loglikelihood value,
    $\mathcal{L}^{(i)}$. \label{getLL}
    \vc
  \item Choose a ``better'' estimate of the parameters, $\hat{\theta}^{(i + 1)}$, and repeat Steps \ref{rowContrib} and
    \ref{getLL}. \label{updateTheta}
    \vc
  \item Repeat Steps \ref{rowContrib} -- \ref{updateTheta} until the change between $\mathcal{L}^{(i - 1)}$ and
    $\mathcal{L}^{(i)}$ falls below some trivially small threshold.
    \vc
  \item Take $\hat{\theta}^{(i)}$ as your estimate.
  \end{enumerate}
...
\begin{frame}[fragile]{ML Example}
  ...
  It turns out that this function is readily available in R via the \pkg{mvtnorm} package:
...
dat1 <- diabetes |> select(bmi, ldl, glu) |> as.matrix()
...
\begin{frame}[fragile]{ML Example}
  ...
  Estimate the same parameters using \pkg{lavaan}::\src{lavCor()}.

<<>>=
## Estimate the parameters:
fit <- diabetes |> 
    select(bmi, ldl, glu) |> 
    lavCor(meanstructure = TRUE, output = "sampstat")
...
muMat           <- rbind(muHat, muHat2)
rownames(muMat) <- c("Manual", "Lavaan")

print(xtable(muMat, digits = 2, caption = "Estimated Means"), booktabs = TRUE)
@ 
...
print(xtable(sigmaHat, digits = 2, caption = "Manual Covariance Matrix"), 
      booktabs = TRUE)
@ 
...
<<echo = FALSE, results = "asis">>= 
print(xtable(sigmaHat2, digits = 2, caption = "Lavaan Covariance Matrix"), 
      booktabs = TRUE)
@
...
\begin{frame}[fragile]{FIML Example}
  ...
  First, punch some holes in our example data.
  ...
  \begin{columns}[onlytextwidth]
    \begin{column}{0.5\textwidth}
      ...
      \item We'll define \Sexpr{nPats} different version of $\mu$ and $\Sigma$.
        \vc
      \item We'll calculate each individual loglikelihood contribution using the appropriate flavor of $\mu$ and
        $\Sigma$. 
      \end{itemize}
...
\begin{frame}[fragile]{FIML Example}
  ...
  To use FIML in \pkg{lavaan}, we need only specify \src{missing = "fiml"}.
  \vc
  \begin{itemize}
  \item Do the same analysis via \pkg{lavaan}::\src{cfa()}.
  \end{itemize}
...
print(xtable(sigmaHat1, 
             digits  = 2, 
             caption = "Manual Covariance Matrix"), 
      booktabs = TRUE)
@ 
...
print(xtable(sigmaHat2, 
             digits  = 2, 
             caption = "Lavaan Covariance Matrix"), 
      booktabs = TRUE)
@
...
\begin{frame}[fragile]{CFA Example}
  ...
  It is very easy to apply FIML to latent variable models in \pkg{lavaan}.

<<>>=
## Estimate the CFA from the beginning of the lecture using FIML:
...
\begin{frame}{FMI with FIML}
  ...
  \vc
  \begin{itemize}
  \item The FMI is calculated using the method described by \citet{savaleiRhemtulla:2012}.
  \end{itemize}
  \vb
  \citet{savaleiRhemtulla:2012} take an information-theoretic approach to defining the FMI.
  \begin{itemize}
  \item Based on the \emph{Missing Information Principle} of \citet{orchardWoodbury:1972}
    \vc  
  \item Their FMI estimates the ratio of missing to complete information for each parameter.
  \end{itemize}
  \vb
  You can use this method to compute the FMI for sufficient statistics via the \pkg{semTools}::\src{fmi()} function.
...
\begin{frame}[allowframebreaks]{Satisfying the MAR Assumption}
  ...
  \end{itemize}
  \vb  
  When the MAR predictors are not substantively interesting variables, naively included them in the analysis model can
  change the model's meaning.
...
\begin{frame}{Saturated Correlates Technique}
  ...
  \citet{graham:2003} developed the \emph{saturated correlates} approach to meet two desiderata:
  \vc
  \begin{enumerate}
  \item Satisfy the MAR assumption by incorporating MAR predictors into the analysis model.
    \vc
  ...
  \item Allow every MAR predictor to covary with all other MAR predictors.
    \vc
  \item Allow every MAR predictor to covary with all observed variables in the analysis model (or their residuals).
  \end{enumerate}
...
\begin{frame}[fragile]{Saturated Correlates Example}
  ...
  We can use the \src{lavaan.auxiliary()} function from \pkg{semTools} (or one of its wrappers) to streamline the
  analysis.
...
\begin{frame}[fragile]{Saturated Correlates Example}
  ...
  The \src{cfa.auxiliary()} function has automatically added the following paths to our model.

   \begin{columns}[onlytextwidth]
    \begin{column}{0.3\textwidth}
      
<<echo = FALSE>>=
modString <- lavExport(fimlOut2, export = FALSE) |> cat() |> capture.output()
grep("age ~~ age", modString, value = TRUE) |> cat(sep = "\n")
@ 
...
<<echo = FALSE>>=
grep("age ~~ male", modString, value = TRUE) |> cat(sep = "\n")
@ 
...
<<echo = FALSE>>=
grep("male ~~ male", modString, value = TRUE) |> cat(sep = "\n")
@ 
...
<<echo = FALSE>>=
grep("age ~~ A\\d", modString, value = TRUE) |> cat(sep = "\n")
@ 
...
<<echo = FALSE>>=
grep("age ~~ O\\d", modString, value = TRUE) |> cat(sep = "\n")
@ 
...
<<echo = FALSE>>=
grep("male ~~ A\\d", modString, value = TRUE) |> cat(sep = "\n")
@ 
...
<<echo = FALSE>>=
grep("male ~~ O\\d", modString, value = TRUE) |> cat(sep = "\n")
@ 
...
compOut <- cfa(cfaMod, data = bfi0$complete, std.lv = TRUE)
                                        ...
                                        #saveRDS(list(bfi0 = bfi0, 
                                        #             naiveOut = naiveOut,
                                        #             miOut = miOut,
                                        #             fimlOut = fimlOut,
                                        #             fimlOut2 = fimlOut2,
                                        #             compOut = compOut),
                                        #        file = "../data/objects.rds")

what <- "agree~~open"

est <- c(coef(compOut)[what],
         coef(naiveOut)[what],
         coef(miOut)[what],
         coef(fimlOut)[what],
         coef(fimlOut2)[what])

fmi <- c(getFmi(miOut, what), 
         getFmi(fimlOut, what), 
...
xtable(tabData, caption = "Latent Covariances", align = "rrrrrrr") |>
...
\begin{frame}[allowframebreaks]{References}
  ...
  \bibliography{../../bibtex/missing_data_refs.bib}
