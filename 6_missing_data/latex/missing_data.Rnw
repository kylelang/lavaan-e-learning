...
\begin{frame}[allowframebreaks]{Good Missing Data Methods}
  ...
  \item Replace the missing values with $M$ plausible estimates
    \vc
    \begin{itemize}
    \item Essentially, a repeated application of stochastic, regression-based 
      prediction to synthesize replacements for the missing data.
      \vc
...
Training this model on the first $N = 400$ observations from the \emph{diabetes}
data produces the following fitted model:
\begin{align*}
...
\begin{itemize}
\item Type I error rates for PN-Type MI improve on single stochastic regression 
  imputation, but they're still too high.
\end{itemize}
...
\begin{frame}{Setting Up Proper MI}
  ...
  $\beta$ and $var(\varepsilon) = \sigma^2$.
  \begin{itemize}
  \item \rmsc{Question:} Why do we only sample $\sigma^2$ and not 
    $\varepsilon$?
  \end{itemize}
  \pause
  \va
  For an imputation model with a normally distributed outcome and uninformative 
  priors, we need to specify two distributions:
  \begin{enumerate}
...
\begin{frame}{PPD of the Missing Data}
  ...
  \vc
  \begin{itemize}
  \item This is the distribution from which we sample replacements for the 
    missing values (i.e., imputations).
    \vc
...
\begin{frame}{Fraction of Missing Information}
  ...
  The \emph{fraction of missing information} (FMI) is a crucial effect size
  measure for missing data problems.
...
\begin{frame}[fragile]{Example: Analysis \& Pooling}
  ...
  We can use the \src{runMI()} function from the \pkg{semTools} package (or one
  of its wrappers) to analyze the multiply imputed datasets and pool results.
...
<<echo = FALSE>>=
partSummary(miOut, 1, fmi = TRUE, drops = c("df", "riv"))
@ 
...
<<echo = FALSE>>=
partSummary(miOut, 2:3, 1:17, fmi = TRUE, drops = c("df", "riv"))
@ 
...
\begin{frame}[fragile]{Model Fit}
  ...
  When we submit the \src{lavaan.mi} object created by \src{runMI()} to the 
  \src{fitMeasures()} function, we can specify the type of pooling rule.
...
%------------------------------------------------------------------------------%
\comment{%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{MV Testing via Model Comparison}

  We can also do an analagous test via a model comparison.
  
<<>>=
## Define constraints directly in the model syntax:
...
## Test the constraints via a nested model comparison:
anova(miOut4, miOut3, test = "D3")
@ 
...
\begin{frame}{Likelihood Function Visualized}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \vc
      \begin{itemize}
        \item What loglikelihoods do we get by varying the value of 
          $\hat{\beta}_{glu}$ while keep all other parameters fixed at their ML 
          estimates?
      \end{itemize}
...
\begin{frame}{Steps of ML}
  ...
  \item Sum the individual loglikelihood contributions from Step 
    \ref{rowContrib} to find the loglikelihood value, $\mathcal{L}^{(i)}$. 
    \label{getLL}
    \vc
  ...
  \item Repeat Steps \ref{rowContrib} -- \ref{updateTheta} until the change 
    between $\mathcal{L}^{(i - 1)}$ and $\mathcal{L}^{(i)}$ falls below some 
    trivially small threshold.
    \vc
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \item We'll define \Sexpr{nPats} different version of $\mu$ and $\Sigma$.
        \vc
      \item We'll calculate each individual loglikelihood contribution using 
...
\begin{frame}[fragile]{Saturated Correlates Example}
  ...
  We can use the \src{lavaan.auxiliary()} function from \pkg{semTools} (or one 
...
\begin{frame}[fragile]{Saturated Correlates Example}
  ...
  The \src{cfa.auxiliary()} function has automatically added the following paths
  to our model.
...
\begin{frame}[allowframebreaks]{References}
  ...
  \bibliography{../../bibtex/winter_school_refs.bib,../../bibtex/missing_data_papers.bib}
