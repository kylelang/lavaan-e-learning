...
In the next two questions, we'll consider one such hypothesis test.
...
- What is the conculsion of this test?
...
**Test the hypothesis from \@ref(waldMi) using a likelihood ratio test.**

- Estimate a separate restricted SEM.
...
To keep things simple, we're only going to estimate a simple mediation model 
wherein *Western beauty standards* predict *preoccupation with food*, and 
*drive for thinness* mediates this relation.
...
- Include defined parameters to quantify the indirect and total effects.
...
samples to test the indirect effects.

The best method for combining these two layers of sampling is not immediately 
obvious, but the literature does provide some guidance. [Wu and Jia (2013)][wu_jia_2013] recommend an approach that they call *MI(Boot)* that entails nesting the 
bootstrapping within the MI as follows:

1. Impute the missing data $M$ times.
1. For each of these $M$ imputed datasets, estimate your mediation model using 
$B$ bootstrap samples.
1. Combine the $M \times B$ resulting estimates of the indirect effect into a 
single sample with $N = M \times B$ observations.
1. Use this aggregated sample to construct CIs and test for mediation.
...
- The following three code chunks do so (in a not-so-elegant way).

---

First, we'll define a function that will take a fitted lavaan object (`lavObj`) 
and return the estimates of all defined parameters specified therein.
...
- We'll generate B = 1000 boostrap samples for each of these 20 imputed datasets.
...
from each one of our fitted models. 

```{r miBoot, include = TRUE, echo = TRUE, cache = TRUE}
## Fit the models:
...
sample with $N = M \times B$ observations.
...
non-parametric (as in bootstrapping), the Monte Carlo method leverages the known 
sampling distributions of $a$ and $b$ to synthesize an empirical sampling 
distribution of their product (i.e., the indirect effect). The Monte Carlo method
entailsthe following procedure.

1. Estimate the mediation model on the full dataset.
   - When analyzing MI data, this model is estimated in the usual way, and the 
   parameter estimates used in the following steps are the pooled MI estimates.
$\vb$   
...
distribution defined above.
   - Although we need to generate a very large sample to get an accurate estimate 
   of the distribution, generating such a sample is computationally trivial and 
   very fast.
$\vb$
...
distribution, and summarize this distribution as you would when using bootstrapping.
...
```{asis}
- The parameter estimates all look reasonable, and the model fits the data well.
- None of the FMI values are especially large, and they tend to be a little 
smaller than the FMI values produced by MI (as shown in the following figure).
```
...
incomplete variables here. The total number of response patterns increases when 
considering the full dataset.

Generally, the model looks good. The parameter estimates all seem sensible, and 
...
are MNAR, and the model estimates (although nice looking), and probably biased.
...
- Keep all other model parameterization and estimation settings the same above.
...
```{asis}
However, I honestly cannot figure out what's wrong, and the residual covariance 
matrix itself looks fine (i.e., no Heywood cases, no extreme values, all 
covariances are legal according to the relevant triangle inequalities). So, 
I'm willing to tentatively trust these estimates.
```
...
relatively small amount of missing data.

One interesting difference is that the number of missing data patterns given in 
the output now matches the number we computed for the entire dataset. Of course, 
this is also natural since we are now using every variable from the EA dat in 
the model.
```
...
- Did anything unusual/unexpected happen during model estimation?
   - If you do see some unusual behavior, can you see what's happening?
...
```{asis}
The paramter estimates are very similar to those produced by MI, and the 
inferential conclusions are all the same. The FMI estimates for the FIML-based 
regression slopes are quite a bit smaller than their MI-based counterpart, though.
```
...
a. Is the indirect effect through *Drive for Thinness* significant according to 
the 95% BC CI?
