...
```{r}
summary(semFit1, fmi = TRUE) %>% print()
```

```{r semRes1, echo = FALSE, results = "asis"}
tmp <- summary(semFit1, fmi = TRUE) %>% quiet()
...
```{r, echo = FALSE}
tmp <- lavTestWald.mi(semFit1, cons, test = "D1")
...
```{r, echo = FALSE}
tmp <- anova(semFit1, semFit2, stat = "D3")
...
```{r, eval = FALSE}
library(coxed) # To calculate BCa intervals

## Define a function that will take a fitted lavaan object ('lavObj') and return 
## the estimates of all defined parameters:
getDefParEstimates <- function(lavObj) {
  ...
  ## Compute the defined parameters on each bootstrapped sample, and return the 
  ## result:
  apply(cf, 1, getDefPar)
}

## Fit 'semMod3' to each of the 20 imputed datasets in 'eaImp'.
## Generate B = 1000 boostrap samples for each of these 20 imputed datasets.
tmp <- semList(semMod3, 
               dataList  = eaImp, 
               std.lv    = TRUE, 
               se        = "boot", 
...
## Aggregate all of the bootstrapped estimates for each defined parameter into a 
## single sample with N = M * B observations:  
est <- tmp@funList %>% do.call(cbind, .) %>% t()

## Use the coxed::bca() function to compute BCa CIs for each defined parameter:
apply(est, 2, bca)
```
...
**Use the Monte Carlo simulation approach to test the mediation hypothesis 
described above.**
