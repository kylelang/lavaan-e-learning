...
- Do these plots suggest convergence?
 ...
 <!-- , animation.hook = "gifski", cache = TRUE} -->
...
from the **lavaan.mi** package to implement both the analysis and pooling phases.

---

The **lavaan.mi** package contains most of the MI-related functionality that was
orginally part of **semTools**. At this point (2024-07-12), **lavaan.mi** isn't
available on CRAN, so we'll need to install the development version from GitHub.

Run the following code to install **lavaan.mi**. You will need to have the
**remotes** package installed to run this code.

```{r install_lavaan_mi, eval = FALSE}
remotes::install_github("TDJorgensen/lavaan.mi")
```

Now, we can use **lavaan.mi**::`cfa.mi()` to fit our models to the list of
multiply imputed datasets from \@ref(fillMissing).
...
library(lavaan.mi)
miCfa <- lavaan.mi::cfa.mi(cfaMod, data = eaImp, std.lv = TRUE, meanstructure = TRUE)
```
